"""
xAquaticRisk Analysis Server — analysis & map explorer web interface.

Standalone web server for running analysis workflows, exploring map geometries,
and querying timeseries data from simulations launched by the main controlpanel.

Requires: XAQ_RUN_DIR env var pointing to the shared run/ folder (same folder
accessed by xAquaticRisk controlpanel on a separate instance).

Endpoints
~~~~~~~~~
    Static
        GET  /                          → index.html (analysis + explorer UI)
    Runs / Monitoring
        GET  /api/controlpanel/status   → server status info
        GET  /api/runs                  → list of all simulation runs
        GET  /api/runs/<id>             → detailed run info
        GET  /api/runs/<id>/log/<name>  → tail parsed log entries
        GET  /api/analysis/default-run-dir  → configured run directory
    Analysis
        GET  /api/analysis-portable-check   → embedded Python runtime status
        GET  /api/analysis/runtime-config   → analysis runtime mode/URL
        GET  /api/analysis/runs         → experiments with MC folders
        GET  /api/analysis/exposure-models  → available exposure model outputs
        GET  /api/analysis/status/<id>  → analysis job status
        GET  /api/analysis/outputs/<id> → analysis job output summary
        GET  /api/analysis/table/<id>   → pecsw_table.json from job output
        GET  /api/analysis/file/<id>/<filename>  → serve analysis output files
        POST /api/analysis/start        → launch analysis workflow
    Map Explorer
        GET  /api/map-explorer/runs     → experiments available for mapping
        POST /api/map-explorer/geometry → load reach + LULC geometries
        POST /api/map-explorer/timeseries → fetch reach timeseries

Usage::

    python server.py                       # default port 8091 (requires XAQ_RUN_DIR env)
    python server.py --port 9000            # custom port
    python server.py --run-dir C:/other     # custom run folder (overrides XAQ_RUN_DIR)
"""
...
def main():
    ap = argparse.ArgumentParser(description="xAquaticRisk Analysis Server")
    ap.add_argument("--port", type=int, default=PORT)
    ap.add_argument("--run-dir", default="", help="Run folder (defaults to XAQ_RUN_DIR env var)")
    args = ap.parse_args()

    # Determine run_dir: prefer command-line arg, fall back to env var, fail if neither
    run_dir = args.run_dir.strip() if args.run_dir else os.environ.get(RUN_DIR_ENV, "").strip()
    if not run_dir:
        print(f"[analysis-server] ERROR: No run directory specified!")
        print(f"[analysis-server] Either:")
        print(f"[analysis-server]   1. Set environment variable: set {RUN_DIR_ENV}=<path>")
        print(f"[analysis-server]   2. Pass command-line argument: --run-dir <path>")
        sys.exit(1)

    run_dir_abs = os.path.abspath(run_dir)
    if not os.path.isdir(run_dir_abs):
        print(f"[analysis-server] ERROR: Run directory does not exist: {run_dir_abs}")
        sys.exit(1)

    ControlPanelHandler.run_root = run_dir_abs

    try:
        _acquire_single_instance_lock(args.port)
    except RuntimeError as exc:
        print(f"[analysis-server] {exc}")
        sys.exit(1)
    ...
    try:
        ...
        print("=" * 60)
        print("  xAquaticRisk Analysis Server")
        print("=" * 60)
